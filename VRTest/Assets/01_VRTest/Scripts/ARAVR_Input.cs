...
public static class ARAVR_Input
{
    ...
    public enum Button 
    {
        ...
        Thumbstick = OVRInput.Button.PrimaryThumbstick,         /**< VR 컨트롤러의 조이스틱 클릭 버튼을 매핑 */
        IndexTrigger = OVRInput.Button.PrimaryIndexTrigger,     /**< VR 컨트롤러의 PrimaryIndexTrigger 버튼을 매핑 */
        HandTrigger = OVRInput.Button.PrimaryHandTrigger        /**< VR 컨트롤러의 PrimaryHandTrigger 버튼을 매핑 */
#endif
    }
     ...
     */
    #region 컨트롤러에 진동 호출하기
    public static void PlayVibration(float duration, float frequency, 
        float amplitude, Controller hand)
    {
#if TARGET_DEVICE_OCULUS
        // 싱글턴 코루틴 인스턴스의 null 체크를 왜 여기서 함
    ...
    /**
     * @brief 컨트롤러의 진동 기능을 구현하기 위한 코루틴 함수
     * @panam duration : 얼마나 오래 진동할 것인지 받아온다
     * @panam frequency : 얼마나 빠르게 진동할 것인지 받아온다
     * @panam amplitude : 얼마나 세게 진동할 것인지 받아온다
     * @panam hand : 어느 컨트롤러를 진동할 것인지 받아온다
     */
    #region 컨트롤러 진동 코루틴
...
//! AVAVRInput 클래스에서 사용할 코루틴 객체
public class CoroutineInstance : MonoBehaviour 
{
    /**< ARAVRInput 기능을 사용하는 동안 메모리에 인스턴스화 할 코루틴 객체 변수이다 */
